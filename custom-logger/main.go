...
func main() {
	...
	if err := botToken.LoadFromConfig("receive-and-send/config.yaml.demo"); err != nil {
		log.Fatalln(err)
	}
	fmt.Println("AppID", botToken.AppID)
	fmt.Println("accessToken", botToken.AccessToken)
	// 初始化 openapi
	api := botgo.NewOpenAPI(botToken).WithTimeout(3 * time.Second)
	// 获取 websocket 信息
	wsInfo, err := api.WS(ctx, nil, "")
	fmt.Printf(wsInfo.URL)
	if err != nil {
		log.Fatalln(err)
	}
	// 根据不同的回调，生成 intents
...
func ATMessageEventHandler(api openapi.OpenAPI) websocket.ATMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSATMessageData) error {
		...
		fmt.Println("clear input content is: %s", input)
		return nil
	}
}
