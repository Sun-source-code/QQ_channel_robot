...
func main() {
	...
	if err := botToken.LoadFromConfig(getConfigPath("config.yaml.demo")); err != nil {
		log.Fatalln(err)
	}
...
func ATMessageEventHandler(idiom map[string]string) websocket.ATMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSATMessageData) error {
		input := strings.ToLower(message.ETLInput(data.Content))
		return processor.ProcessMessage(input, data, idiom)
	}
}
